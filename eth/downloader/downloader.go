...
var (
	...
	fsMinFullBlocks        = 256             // Number of blocks to retrieve fully even in fast sync
)
	...
	for {
		...
		if atomic.LoadInt32(&d.committed) == 0 {
			...
			if height := latest.Number.Uint64(); height >= pivot.Number.Uint64()+2*uint64(fsMinFullBlocks)-uint64(reorgProtHeaderDelay) {
				...
				log.Warn("Pivot became stale, moving", "old", pivot.Number.Uint64(), "new", height+uint64(reorgProtHeaderDelay))
				pivot = results[len(results)-1].Header // must exist as lower old pivot is uncommitted
